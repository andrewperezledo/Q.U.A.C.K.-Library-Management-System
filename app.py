...
from DatabaseTools.databasetools import loginFunction
from DatabaseTools.databasetools import userSearch
...
def create_user():
    if session["usertype"] == "employee" or session["usertype"] == "admin": # This line only lets admin or employees use this page
...
def login_page():
    if request.method == 'POST':
        ...
        result = loginFunction(username, password)
        if result == "Login Successful.":
            userdata = userSearch(username)
...
def home_user():
    if "username" in session:
        ...
        if session["usertype"] == "member":
            ...
            return render_template('test_employeeonly.html')
